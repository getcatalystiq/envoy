...
function TranscriptEventCard({ event }: { event: TranscriptEvent }) {
  ...
  const getEventColor = (type: string) => {
    switch (type) {
      ...
      case 'run_started': return 'bg-gray-100 text-gray-800';
  ...
  return (
    <div className="border border-gray-200 rounded-md bg-white">
      <button className="w-full flex items-center justify-between p-2 text-left hover:bg-gray-50" onClick={() => setExpanded(!expanded)}>
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          {label && <span className="text-sm font-mono text-gray-700 flex-shrink-0">{label}</span>}
          {preview && <span className="text-sm text-gray-600 truncate">{preview}</span>}
        </div>
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        {runs.length === 0 ? (
          <div className="text-center py-8"><Activity className="w-12 h-12 mx-auto text-gray-300 mb-4" /><p className="text-gray-500">No AI activity recorded yet.</p></div>
        ) : (
          <div className="space-y-2">
            {runs.map((run) => (
              <button key={run.id} className="w-full flex items-center justify-between p-4 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors text-left" onClick={() => loadDetail(run.id)}>
