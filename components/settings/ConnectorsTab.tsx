...
export function ConnectorsTab() {
  ...
  const handleOAuthConnect = async (slug: string) => {
    ...
    setError(null);
    try {
      const data = await api.post<{ redirect_url: string }>(`/agentplane/connectors/${slug}/oauth`);
      if (data.redirect_url) {
        const popup = window.open(data.redirect_url, 'agentplane_oauth', 'width=600,height=700,popup');
        if (!popup) throw new Error('Popup blocked. Please allow popups for this site.');
        const pollTimer = setInterval(() => {
          if (popup.closed) { clearInterval(pollTimer); loadConnectors(); setConnectingSlug(null); }
        }, 1000);
      }
    } catch (err) {
      setError((err as Error).message);
  ...
  const handleSaveApiKey = async () => {
    ...
    setError(null);
    try {
      await api.post(`/agentplane/connectors/${apiKeyDialog}/api-key`, { api_key: apiKeyValue });
  ...
  return (
    <Card>
              ...
              <div key={connector.slug} className="flex flex-col p-4 bg-muted rounded-lg border">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  {isConnected(connector) ? (
                    ...
                    <Badge variant="secondary" className="gap-1"><XCircle className="w-3 h-3" />Not connected</Badge>
                  )}
                </div>
                <div className="flex items-center gap-2 mt-auto">
                  {isConnected(connector) ? (
                    <Button variant="outline" size="sm" className="w-full" onClick={() => setDisconnectConfirm(connector.slug)}>Disconnect</Button>
                  ) : connector.authScheme === 'API_KEY' ? (
                    <Button size="sm" className="w-full" onClick={() => { setApiKeyDialog(connector.slug); setApiKeyValue(''); }}><Key className="w-4 h-4 mr-1" />Set API Key</Button>
                  ) : connector.authScheme === 'NO_AUTH' ? (
                    <Badge variant="outline" className="w-full justify-center py-1">Ready</Badge>
                  ) : (
                    <Button size="sm" className="w-full" onClick={() => handleOAuthConnect(connector.slug)} disabled={connectingSlug === connector.slug}>
                      {connectingSlug === connector.slug ? (<><Loader2 className="w-4 h-4 animate-spin mr-1" />Connecting...</>) : (<><PlugZap className="w-4 h-4 mr-1" />Connect</>)}
                    </Button>
                  )}
                </div>
              </div>
      ...
      <Dialog open={!!apiKeyDialog} onOpenChange={() => setApiKeyDialog(null)}>
        <DialogContent>
          <DialogHeader><DialogTitle>Set API Key</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label htmlFor="api-key">API Key for {apiKeyDialog}</Label><Input id="api-key" type="password" value={apiKeyValue} onChange={(e) => setApiKeyValue(e.target.value)} placeholder="Enter API key..." /></div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setApiKeyDialog(null)}>Cancel</Button>
            <Button onClick={handleSaveApiKey} disabled={!apiKeyValue || isSavingKey}>
              {isSavingKey ? (<><Loader2 className="w-4 h-4 animate-spin mr-2" />Saving...</>) : 'Save'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
