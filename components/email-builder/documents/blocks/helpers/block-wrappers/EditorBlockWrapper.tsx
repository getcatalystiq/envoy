...
import { setSelectedBlockId, useDocument, useSelectedBlockId, useReadOnly } from '../../../editor/EditorContext';
  ...
  return (
    <div
      className={cn(
        ...
        hasPersonalization && !readOnly && 'ai-shimmer-border'
      )}
      onMouseEnter={(ev) => {
        if (!readOnly) {
          setMouseInside(true);
    ...
    >
      {hasPersonalization && !readOnly && sparkles.map((sparkle) => (
        <span
          key={sparkle.id}
      ...
      {isSelected && !readOnly && <TuneMenu blockId={blockId} />}
      {children}
    </div>
  );
}
