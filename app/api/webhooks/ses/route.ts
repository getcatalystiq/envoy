...
export async function POST(request: Request) {
  ...
  // Verify TopicArn matches expected SNS topic to prevent spoofed notifications
  const expectedTopicArn = getEnv().SES_NOTIFICATION_TOPIC_ARN;
  if (expectedTopicArn) {
    const messageTopicArn = message.TopicArn as string;
    if (messageTopicArn !== expectedTopicArn) {
      return jsonResponse({ error: "Unexpected SNS topic" }, 403);
    }
  }
