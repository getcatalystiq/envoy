...
import { api, type Sequence, type SequenceStatus, type CreateSequenceInput } from '@/lib/api';
import {
  Plus,
  GitBranch,
  MoreVertical,
      ...
      {filteredSequences.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredSequences.map((sequence) => (
            <Card key={sequence.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => router.push(`/sequences/${sequence.id}`)}>
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); openEditDialog(sequence); }}>
                        <Pencil className="w-4 h-4 mr-2" />Edit
                      </DropdownMenuItem>
                      {sequence.status === 'draft' && (
                        <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleActivate(sequence); }}>
                          <Play className="w-4 h-4 mr-2" />Activate
                        </DropdownMenuItem>
                      )}
                      {sequence.status === 'active' && (
                        <>
                          <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); handlePause(sequence); }}>
                            <Pause className="w-4 h-4 mr-2" />Pause
                          </DropdownMenuItem>
                          <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); handleArchive(sequence); }}>
                            <Archive className="w-4 h-4 mr-2" />Archive
                          </DropdownMenuItem>
                        </>
                      )}
                      {sequence.status !== 'active' && (
                        <DropdownMenuItem onClick={(e: React.MouseEvent) => { e.stopPropagation(); setSequenceToDelete(sequence); setShowDeleteDialog(true); }} className="text-red-600">
                ...
                <div className="mb-2 flex items-center gap-2">
                  <Badge variant="outline">
                    {sequence.target_type_id ? targetTypes.find((t) => t.id === sequence.target_type_id)?.name ?? 'Unknown type' : 'All targets'}
                  </Badge>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="e.g., Welcome Series, Follow-up Campaign" value={newSequence.name} onChange={(e) => setNewSequence({ ...newSequence, name: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="target-type">Target Type (optional)</Label>
              <Select value={newSequence.target_type_id || 'all'} onValueChange={(value: string) => setNewSequence({ ...newSequence, target_type_id: value === 'all' ? undefined : value })}>
                <SelectTrigger><SelectValue placeholder="All targets" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All targets</SelectItem>
                  {targetTypes.map((type) => (<SelectItem key={type.id} value={type.id}>{type.name}</SelectItem>))}
                </SelectContent>
              </Select>
            </div>
          </div>
          <DialogFooter>
            ...
            <Button onClick={handleCreateSequence} disabled={!newSequence.name.trim() || isCreating}>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <Input id="edit-name" placeholder="e.g., Welcome Series, Follow-up Campaign" value={editSequence.name} onChange={(e) => setEditSequence({ ...editSequence, name: e.target.value })} />
            </div>
            <div className="space-y-2">
              <Label htmlFor="edit-target-type">Target Type</Label>
              <Select value={editSequence.target_type_id || 'all'} onValueChange={(value: string) => setEditSequence({ ...editSequence, target_type_id: value === 'all' ? undefined : value, is_default: value === 'all' ? false : editSequence.is_default })}>
                <SelectTrigger><SelectValue placeholder="All targets" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="all">All targets</SelectItem>
                  {targetTypes.map((type) => (<SelectItem key={type.id} value={type.id}>{type.name}</SelectItem>))}
            ...
            {editSequence.target_type_id && (
              <div className="flex items-center justify-between rounded-lg border p-4">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">New targets of this type will be auto-enrolled</p>
                </div>
                <Switch id="is-default" checked={editSequence.is_default} onCheckedChange={(checked) => setEditSequence({ ...editSequence, is_default: checked })} disabled={sequenceToEdit?.status !== 'active'} />
              </div>
            )}
            {editSequence.target_type_id && sequenceToEdit?.status !== 'active' && (
              <p className="text-xs text-muted-foreground">Only active sequences can be set as default</p>
            )}
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowEditDialog(false)}>Cancel</Button>
            <Button onClick={handleEditSequence} disabled={!editSequence.name.trim() || isEditing}>
