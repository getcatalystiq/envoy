...
export default function DesignTemplatesPage() {
  ...
  if (error) {
    return (<div className="space-y-6"><div className="text-center py-12 text-destructive">{error}</div></div>);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {templates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {templates.map((template) => (
            <Card key={template.id} className={`hover:shadow-md transition-shadow cursor-pointer overflow-hidden ${template.archived ? 'opacity-60' : ''}`} onClick={() => router.push(`/design-templates/${template.id}`)}>
              <div className="p-3 pb-0"><TemplatePreview content={template.builder_content} /></div>
              <div className="px-3 py-2">
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-base truncate">{template.name}</h3>
                    {template.archived && <Badge variant="outline" className="text-xs mt-1">Archived</Badge>}
                    {template.description && <p className="text-sm text-muted-foreground line-clamp-2 mt-1">{template.description}</p>}
                  </div>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem onClick={(e) => { e.stopPropagation(); router.push(`/design-templates/${template.id}`); }}>
                        <Edit className="h-4 w-4 mr-2" />Edit
                      </DropdownMenuItem>
                      {template.archived ? (
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); handleUnarchive(template.id); }}>
                          <Archive className="h-4 w-4 mr-2" />Unarchive
                        </DropdownMenuItem>
                      ) : (
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Create Design Template</DialogTitle></DialogHeader>
          <div className="space-y-4">
            <div><Label htmlFor="new-name">Name</Label><Input id="new-name" value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Welcome Email Template" /></div>
            <div><Label htmlFor="new-description">Description (optional)</Label><Textarea id="new-description" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder="Template for welcome emails..." /></div>
          </div>
          <DialogFooter>
